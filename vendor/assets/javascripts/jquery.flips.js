  ...
  // set this.$el to #flip, call _init function
	...
	$.Flips.prototype = {
	  ...
	  // initialize the other functions
		...
		// monitors window size, subtracted sidebar length to fit
		...
		// this.state is from _getState function, goes to this.currentPage if this.state is undefined
		_goto	: function() {	
			var page = ( this.state === undefined ) ? this.currentPage : this.state;
			// goes back to 0 if page isn't valid
		...
		// flipside right to left or left to right, increment or decrement currentPage
		...
		// append overlay to .outer and .inner
